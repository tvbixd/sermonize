...
export default function SignUpScreen() {
  ...
  const { signUpWithEmail, signInWithIdToken } = useAuth();
    ...
    return (
      <SafeAreaView style={s.container} edges={['top', 'bottom']}>
        <View style={s.successCenter}>
          ...
          <Text style={[s.subtitle, { maxWidth: 280 }]}>
            We sent a confirmation link to {email}. Tap it to activate your account, then come back here.
          </Text>
          <TouchableOpacity
            style={[s.primaryBtn, { backgroundColor: t.accentBlue, width: '100%' }]}
            activeOpacity={0.85}
      ...
      >
        ...
        >
          <View style={s.header}>
            ...
            <Text style={s.subtitle}>Start capturing your sermons with Scribe.</Text>
          </View>
          ...
          <View style={s.footer}>
            <Text style={[s.footerText, { color: t.textSecondary }]}>Already have an account?</Text>
            <TouchableOpacity onPress={() => router.replace('/sign-in')} activeOpacity={0.7}>
              <Text style={[s.footerLink, { color: t.accentBlue }]}> Sign In</Text>
            </TouchableOpacity>
          </View>
        </ScrollView>
      </KeyboardAvoidingView>
...
function makeStyles(t: Colors) {
  return StyleSheet.create({
    ...
    successCenter: { flex: 1, alignItems: 'center', justifyContent: 'center', paddingHorizontal: spacing.xl, gap: 16 },
